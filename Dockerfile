...
RUN apk update --no-cache &&\
    apk add --no-cache nano ca-certificates wget nginx git postgresql-dev musl-dev gcc jpeg-dev zlib-dev libxml2-dev libxslt-dev libffi-dev &&\
...
ENV TAIGA_DB_HOST "db"
ENV TAIGA_DB_NAME "taiga"
ENV TAIGA_DB_USER "taiga"
...
# remove without install plugin
RUN apk del gcc musl-dev libxml2-dev
RUN apk add gettext
